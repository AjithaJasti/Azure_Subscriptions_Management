...
export const Creation = (props) => {
  console.log("enrollment acc",props.sampleData.value[0].properties.enrollmentAccounts[0].id);
...
//Getting "rubrik-cloud-health" object ID
  const applicationData = async() => {
    ...
    await fetch('https://graph.microsoft.com/v1.0/servicePrincipals?$search="displayName:rubrik-cloudhealth-Oasis"', appoptions)
...
const options={
  method: "PUT",
    ...
    await fetch(`https://management.azure.com/subscriptions/${subsdata}/providers/Microsoft.Resources/tags/default?api-version=2021-04-01`,options)
    .then((data) => {
      console.log(data);
    })
    .catch((err) => {
...
//Creating Roles
    const roleoptions={
        method: "PUT",
          ...
          body: JSON.stringify({
              "properties": {
                  "roleDefinitionId":  "/subscriptions/" +subsdata + "/providers/Microsoft.Authorization/roleDefinitions/acdd72a7-3385-48ef-bd42-f606fba81ae7",
                  "principalId": objId.value[0].id
              }
          })
    };

    await fetch('https://management.azure.com/subscriptions/' + subsdata +'/providers/Microsoft.Authorization/roleAssignments/'+ uuid() +'?api-version=2015-07-01',roleoptions)
        .then((data) => {
          console.log(data);
        })
        .catch((err) => {
    ...
    <>
      ...
      <div className="divcreateform">
        <form onSubmit={onSubmit} className="createforms">
          ...
          <input required value={values.costcenter} onChange={set("costcenter")} />
